...
#include "io/camera.hpp"  // 新增：相机相关头文件
#include "tasks/auto_aim/aimer.hpp"
#include "tasks/auto_aim/detector.hpp"  // 新增：Detector类头文件
...
// 调整命令行参数：增加传统识别切换选项，移除视频相关参数
const std::string keys =
  "{help h usage ? |                   | 输出命令行参数说明 }"
  "{config-path c  | configs/demo.yaml | yaml配置文件的路径}"
  "{tradition t    | false             | 是否使用传统方法识别}";  // 新增：传统识别切换
...
int main(int argc, char * argv[])
{
  ...
  auto use_tradition = cli.get<bool>("tradition");  // 新增：获取传统识别参数

  tools::Plotter plotter;
  tools::Exiter exiter;

  // 替换视频输入为相机输入
  io::Camera camera(config_path);  // 新增：相机初始化

  // 初始化识别组件（包含传统识别器）
  auto_aim::YOLO yolo(config_path);
  auto_aim::Detector detector(config_path, true);  // 新增：声明Detector对象
  ...
  while (!exiter.exit()) {
    ...
    camera.read(img, timestamp);  // 替换：相机读取图像
    frame_count++;
    ...
    /// 自瞄核心逻辑
    // 姿态数据：如果相机有姿态反馈，需替换为实际数据；此处用默认值
    Eigen::Quaterniond R_gimbal2world(1, 0, 0, 0);  // 修正：显式指定double类型
    solver.set_R_gimbal2world(R_gimbal2world);

    auto yolo_start = std::chrono::steady_clock::now();
    std::list<auto_aim::Armor> armors;
    
    // 新增：根据参数选择识别方式（传统/YOLO）
    if (use_tradition)
    ...
    // 修正：显式指定四元数为double类型
    Eigen::Quaterniond gimbal_q = R_gimbal2world;
    tools::draw_text(
      img,
      fmt::format(
    ...
    // 修正：四元数类型为double
    Eigen::Quaterniond q = R_gimbal2world;
