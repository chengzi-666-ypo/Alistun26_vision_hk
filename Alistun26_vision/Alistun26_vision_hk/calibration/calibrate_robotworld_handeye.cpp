...
const std::string keys =
  ...
  "{config-path c  | configs/calibration.yaml | 主yaml配置文件路径 }"
  "{intrinsic-config i | configs/standard_serial.yaml | 相机内参yaml路径 }"
...
void load(
  const std::string & input_folder, const std::string & config_path, const std::string & intrinsic_path,
  ...
  std::vector<cv::Mat> & tvecs)
{
  ...
  // 兼容读取 R_gimbal2imubody：支持 [9项扁平数组] 或 [[3项],[3项],[3项]] 的二维数组
  try {
    R_gimbal2imubody_data = yaml["R_gimbal2imubody"].as<std::vector<double>>();
  } catch (...) {
    R_gimbal2imubody_data.clear();
    auto node = yaml["R_gimbal2imubody"];
    if (node && node.IsSequence() && node.size() == 3 && node[0].IsSequence()) {
      for (int r = 0; r < 3; ++r) {
        for (std::size_t c = 0; c < node[r].size(); ++c) {
          R_gimbal2imubody_data.push_back(node[r][c].as<double>());
        }
      }
    }
  }
  if (R_gimbal2imubody_data.size() != 9) {
    fmt::print("[warn] R_gimbal2imubody not found or invalid, fallback to identity.\n");
    R_gimbal2imubody_data = {1,0,0, 0,1,0, 0,0,1};
  }

  // 如果主配置缺少相机内参，则从 intrinsic_path 加载
  YAML::Node cam_yaml = yaml;
  if (!yaml["camera_matrix"] || !yaml["distort_coeffs"]) {
    cam_yaml = YAML::LoadFile(intrinsic_path);
  }
  auto camera_matrix_data = cam_yaml["camera_matrix"].as<std::vector<double>>();
  auto distort_coeffs_data = cam_yaml["distort_coeffs"].as<std::vector<double>>();
...
int main(int argc, char * argv[])
{
  ...
  auto intrinsic_path = cli.get<std::string>("intrinsic-config");
  ...
  load(
    input_folder, config_path, intrinsic_path, R_gimbal2imubody_data, R_world2gimbal_list, t_world2gimbal_list,
    rvecs, tvecs);
