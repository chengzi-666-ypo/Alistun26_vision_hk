  ...
  bool to_now)
{
  ...
  // If bullet speed is abnormally small (e.g., bad scaling), fallback to 15 m/s
  if (bullet_speed < 5.0) bullet_speed = 15.0;
  ...
  if (!aim_point0.valid) {
    // Fallback: geometric aim with first candidate point; keep control, do not shoot
    Eigen::Vector3d fb = aim_point0.xyza.head(3);
    double d_fb = std::hypot(fb.x(), fb.y());
    double yaw_fb = std::atan2(fb.y(), fb.x()) + yaw_offset_;
    double pitch_fb = -(std::atan2(fb.z(), std::max(1e-6, d_fb)) + pitch_offset_);
    return {true, false, yaw_fb, pitch_fb};
  }

  Eigen::Vector3d xyz0 = aim_point0.xyza.head(3);
  auto d0 = std::sqrt(xyz0[0] * xyz0[0] + xyz0[1] * xyz0[1]);

  // 安全阈值：过滤距离过近（预测框过大）的误识别
  if (d0 < 0.1) {
    // Too close: fallback to geometric aim to keep tracking (no shoot)
    double yaw_fb = std::atan2(xyz0.y(), xyz0.x()) + yaw_offset_;
    double pitch_fb = -(std::atan2(xyz0.z(), std::max(1e-6, d0)) + pitch_offset_);
    return {true, false, yaw_fb, pitch_fb};
  }
  ...
  if (trajectory0.unsolvable) {
    ...
    // Fallback: keep tracking using geometric aim (no gravity compensation), do not shoot
    double yaw_fb = std::atan2(xyz0.y(), xyz0.x()) + yaw_offset_;
    double pitch_fb = -(std::atan2(xyz0.z(), d0) + pitch_offset_);
    return {true, false, yaw_fb, pitch_fb};
  }
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    if (!aim_point.valid) {
      // Fallback: geometric aim with current predicted point; keep control, do not shoot
      Eigen::Vector3d fb = aim_point.xyza.head(3);
      double d_fb = std::hypot(fb.x(), fb.y());
      double yaw_fb = std::atan2(fb.y(), fb.x()) + yaw_offset_;
      double pitch_fb = -(std::atan2(fb.z(), std::max(1e-6, d_fb)) + pitch_offset_);
      return {true, false, yaw_fb, pitch_fb};
    ...
    if (current_traj.unsolvable) {
      ...
      debug_aim_point.valid = false;
      // Fallback: geometric aim when ballistic solution is unsolvable; keep control, do not shoot
      double yaw_fb = std::atan2(xyz.y(), xyz.x()) + yaw_offset_;
      double pitch_fb = -(std::atan2(xyz.z(), d) + pitch_offset_);
      return {true, false, yaw_fb, pitch_fb};
    }
...
AimPoint Aimer::choose_aim_point(const Target & target)
{
  ...
  // 不考虑小陀螺
  if (std::abs(target.ekf_x()[7]) <= decision_speed_ && target.name != ArmorName::outpost) {
    // 选择在可射击范围内的装甲板
