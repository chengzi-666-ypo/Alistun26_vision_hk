...
public:
  ...
  std::tuple<std::vector<Object>, std::chrono::steady_clock::time_point, cv::Mat> pop();

private:
  ov::Core core;  // 创建OpenVINO Runtime Core对象
  std::shared_ptr<ov::Model> model;
  ov::CompiledModel compiled_model;
  ov::InferRequest infer_request; // 保留用于同步调用
  ov::Tensor input_tensor;
  const int NUM_POINTS = 6;
  double confidence_threshold_; // 成员变量，从yaml读取

  // 异步推理资源池
  const int num_requests = 4;
  std::vector<ov::InferRequest> requests_;
  std::vector<int> idle_indices_;
  ...
  // 忙队列，存储正在进行的任务
  tools::ThreadSafeQueue<Task> busy_queue_{4};
