...
{
static std::atomic<bool> g_decider_watch_file{false};
static std::string g_decider_enemy_color_file_str;

static void decider_watch_enemy_color_file()
{
  const std::string path = "/tmp/robot_enemy_color";
  std::string last;
  while (g_decider_watch_file.load()) {
    ...
    if (ifs) {
      ...
      if (!v.empty() && v != last) {
        ...
        last = v;
        g_decider_enemy_color_file_str = v;
        tools::logger()->info("[Decider] enemy_color updated from /tmp/robot_enemy_color -> {}", v);
      }
    }
...
Decider::Decider(const std::string & config_path) : detector_(config_path), count_(0)
{
  ...
  // 解析 enemy_color，支持 red / blue / both（忽略大小写）
  enemy_color_str_ = yaml["enemy_color"].as<std::string>();
  std::transform(enemy_color_str_.begin(), enemy_color_str_.end(), enemy_color_str_.begin(),
                 [](unsigned char c) { return std::tolower(c); });
  g_decider_enemy_color_file_str = enemy_color_str_;

  if (enemy_color_str_ == "red") {
    enemy_color_ = auto_aim::Color::red;
    tools::logger()->info("[Decider] enemy_color set to red");
  } else if (enemy_color_str_ == "blue") {
    enemy_color_ = auto_aim::Color::blue;
    tools::logger()->info("[Decider] enemy_color set to blue");
  } else if (enemy_color_str_ == "both") {
    // both 模式下不按颜色过滤，仍需给 enemy_color_ 一个默认值以兼容其它代码
    enemy_color_ = auto_aim::Color::red;
    tools::logger()->info("[Decider] enemy_color set to both (no color filtering)");
  } else {
    tools::logger()->error(
      "[Decider] invalid enemy_color '{}', expected 'red', 'blue' or 'both'", enemy_color_str_);
    exit(1);
  }

  // start file watcher thread to allow runtime updates from referee_listener
  g_decider_watch_file.store(true);
  std::thread(decider_watch_enemy_color_file).detach();

  // if the file already exists at startup, prefer it over the YAML
  std::ifstream ifs_init("/tmp/robot_enemy_color");
  if (ifs_init) {
    std::string v;
    std::getline(ifs_init, v);
    if (!v.empty()) {
      std::transform(v.begin(), v.end(), v.begin(), [](unsigned char c) { return std::tolower(c); });
      enemy_color_str_ = v;
      if (enemy_color_str_ == "red") enemy_color_ = auto_aim::Color::red;
      else if (enemy_color_str_ == "blue") enemy_color_ = auto_aim::Color::blue;
      else enemy_color_ = auto_aim::Color::red;
      tools::logger()->info("[Decider] enemy_color overridden at startup from /tmp/robot_enemy_color -> {}", enemy_color_str_);
    }
  }
...
bool Decider::armor_filter(std::list<auto_aim::Armor> & armors)
{
  ...
  // 过滤非敌方装甲板；如果配置为 both 则不过滤颜色
  if (g_decider_enemy_color_file_str != "both") {
    auto_aim::Color current_enemy_color = (g_decider_enemy_color_file_str == "blue") ? auto_aim::Color::blue : auto_aim::Color::red;
    armors.remove_if([&](const auto_aim::Armor & a) { return a.color != current_enemy_color; });
  }
